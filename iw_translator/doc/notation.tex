...
The goal is to determine $a_{i}$, the activation state of strategy $i$, by respecting these constraints:
...
And observe the following optimizations:

\begin{itemize}
    \item Minimize $\sum\limits_i a_i$ to avoid activating extra strategies for
        unwanted desires (usefulness currently being evaluated),
    \item Maximize $\sum\limits_k \sum\limits_i a_{i} (u_{ik} - r_{ik})$, the
        total utility produced.
\end{itemize}
